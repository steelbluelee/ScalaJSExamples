...
object NewtonRaphson {
  ...
  def main01(): Unit = {
    ...
    val answer = Try(t.asInstanceOf[String].toDouble) map { a =>
      {
        ...
        lazy val newtonRaphson: State[Double, Double] = for {
          ...
          result <- if ((xold - xnew) / xold < EPSILON) // format: off
    ...
    js.Dynamic.global.document
      .write(answer.getOrElse("에러").asInstanceOf[scala.scalajs.js.Any])
  }
}
